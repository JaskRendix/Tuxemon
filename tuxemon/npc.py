        ...
        if len(self.monsters) >= self.party_limit:
            ...
            if self.monster_boxes.is_box_full(kennel):
                i = len(
                    [
                        box_id
                        for box_id in self.monster_boxes.get_box_ids()
                        if box_id.startswith(kennel)
                        and self.monster_boxes.is_box_full(box_id)
                    ]
                )
                self.monster_boxes.create_box(f"{kennel}{i}")
                self.monster_boxes.move_monster(
                    kennel, f"{kennel}{i}", monster
                )
                self.monster_boxes.remove_monster_from(kennel, monster)
        else:
